...
@testset "regression tests" begin
    @testset "integration of single peak" begin
        ...
        # note that random numbers in Julia might not be reproducible
        # when switching minor versions, https://docs.julialang.org/en/v1/stdlib/Random/#Reproducibility
        @test pmean(area) ≈ 1.71985533 atol = 1e-8
        @test pstd(area) ≈ 0.249306770 atol = 1e-8
    end
    @testset "integration of two peaks" begin
        ...
        # note that random numbers in Julia might not be reproducible
        # when switching minor versions, https://docs.julialang.org/en/v1/stdlib/Random/#Reproducibility
        @test pmean(area1) ≈ 1.71985533 atol = 1e-8
        @test pstd(area1) ≈ 0.249306770 atol = 1e-8
        @test pmean(area2) ≈ 3.116804434 atol = 1e-8
        @test pstd(area2) ≈ 0.2673814109 atol = 1e-8
    end
end
...
@testset "integrate using Simpsons rule from external package" begin
    ...
    @test pmean(area_s) ≈ 5.8287047 atol = 1e-7
    @test pstd(area_s) ≈ 0.34293816 atol = 1e-7
end
